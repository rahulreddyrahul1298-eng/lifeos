    ...
    <div className="min-h-screen bg-[#FAFBFC]">
      <nav className="bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/" className="text-lg font-black text-gray-900">Life<span className="text-gradient">OS</span></Link>
          <Link href="/dashboard" className="text-sm text-gray-400 hover:text-gray-900 font-medium transition-colors">← Dashboard</Link>
        </div>
      </nav>
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="text-center mb-14">
          ...
          <p className="text-gray-500 text-lg">Get AI insights and spending predictions</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-2xl mx-auto mb-20">
          <div className="card p-8">
            ...
            <div className="space-y-3 mb-8">{["Expense tracking", "Category budgets", "Savings goals", "Debt tracking", "Auto-categorize", "Budget alerts"].map((f,i)=>(
              <div key={i} className="flex items-center gap-3 text-sm text-gray-700"><span className="text-emerald-500 font-bold">✓</span>{f}</div>
            ))}</div>
            <button onClick={()=>router.push("/auth")} className="btn-secondary w-full py-3.5 text-sm">Start Free</button>
          </div>

          <div className="card p-8 ring-2 ring-indigo-500/15 relative overflow-hidden">
            <div className="absolute top-0 right-0 bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-[9px] font-black px-4 py-1.5 rounded-bl-3xl">BEST VALUE</div>
            ...
            <div className="space-y-3 mb-8">{["Everything in Free", "AI insights & predictions", "Spending projections", "Avg daily spend analysis", "Monthly reports", "Priority support"].map((f,i)=>(
        ...
        <div className="max-w-2xl mx-auto mb-16">
          ...
          ].map((f,i)=>(<div key={i} className="card p-5"><p className="text-sm font-bold text-gray-900 mb-1">{f.q}</p><p className="text-sm text-gray-400">{f.a}</p></div>))}</div>
        </div>

        {!isPremium && (
          <div className="text-center pb-10">
            <h3 className="text-2xl font-black text-gray-900 mb-4">Ready to upgrade?</h3>
      ...
      <footer className="border-t border-gray-100 py-8 bg-white"><div className="max-w-4xl mx-auto px-6 text-center text-xs text-gray-400">© 2025 LifeOS</div></footer>
    </div>
  );
}
